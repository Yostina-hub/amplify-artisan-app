    ...
    const aiResponse = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a content moderation AI. Analyze social media posts and identify policy violations.
...
Respond ONLY with valid JSON in this exact format:
{
  ...
  "severity": "none" | "low" | "medium" | "high"
          ...
          {
            role: 'user',
            content: `Analyze this social media post for ${platforms.join(', ')}:\n\n"${content}"\n\nProvide your analysis in JSON format.`
          }
        ],
        temperature: 0.3,
      }),
